...
let onImageGenerationRequest = class onImageGenerationRequest {
    imageGenerationQueue;
    constructor() {
        this.imageGenerationQueue = new Queue();
    }
    TIME_PER_PERSON = 2 * 60 * 1000;
    async main([message]) {
        if (message.channel.id !== CHANNEL_IDS.imageGenerationChannel || message.author.bot || !message.content.toLowerCase().startsWith("generate"))
            return;
        ...
        if (!isNitro && user?.money < imageGenerationCost) {
            await message
                .reply("Você não tem pontos de atividade o suficiente para gerar uma imagem. Você pode ganhar pontos de atividade fazendo RP.\n\n💡 Usuários Nitro podem gerar imagens gratuitamente. Considere apoiar nosso servidor.")
        ...
        if (isAlreadyInQueue) {
            ...
            await message
                .reply(`Você já está na fila. Tempo estimado para a sua vez: {time}. Posição na fila: {position}`
            ...
            execute: async () => {
                ...
                async function generateImage() {
                    try {
                        ...
                        if (isLarge)
                            input.splice(input.indexOf("large"), 1);
                        ...
                        await message.channel.send({
                            content: `{user}, aqui está a imagem gerada com base no prompt {prompt}\n💰{cost}`
                                ...
                                .replace("{cost}", isNitro ? "Por ser um usuário Nitro, você não foi cobrado." : `Você foi cobrado C$${imageGenerationCost} pontos de atividade.`)
...
], onImageGenerationRequest.prototype, "main", null);
onImageGenerationRequest = __decorate([
    Discord()
], onImageGenerationRequest);
export default onImageGenerationRequest;
