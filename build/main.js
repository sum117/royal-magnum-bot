...
import { DiscordAPIError, IntentsBitField, Partials, codeBlock } from "discord.js";
import { Client } from "discordx";
import "dotenv/config";
import lodash from "lodash";
import cron from "node-cron";
import AchievementEmitter, { AchievementEvents, handleBuyCharacterAchievements, handleCharacterCreate, handleCharacterLevelUpAchievements, handleCharacterMessageAchievements, } from "./achievements";
...
// }
function assignSystemChannels(readyClient) {
    const guild = readyClient.guilds.cache.first();
    console.log(`Found guild: ${guild?.name}`);
    if (!guild)
        return;
    bot.systemChannels = guild.channels.cache.filter((channel) => {
        return Object.values(CHANNEL_IDS).includes(channel.id) && channel.isTextBased();
    });
...
async function registerSchedules(readyClient) {
    const isRoleplayingChannel = (channel) => !channel.isDMBased() && channel.isTextBased() && !channel.isThread() && Boolean(channel.parent?.name.startsWith("RP |"));
    const roleplayingChannels = readyClient.guilds.cache.first()?.channels.cache.filter(isRoleplayingChannel);
    if (!roleplayingChannels)
        return;
    cron.schedule(CRON_EXPRESSIONS.EveryFourHours, async () => {
        for (const [_id, rpChannel] of roleplayingChannels)
            await Channel.manageChannelPlaceholder(rpChannel);
    }, { runOnInit: true });
}
async function run() {
...
function logError(error) {
    const errorChannel = bot.systemChannels.get(CHANNEL_IDS.errorLog);
    let message = { label: "", stack: "" };
